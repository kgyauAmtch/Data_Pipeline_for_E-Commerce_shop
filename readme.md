...
Raw ecommerce data files such as orders, order items, and products are generated by the source system.
...
4.The debounce mechanism waits for a configured time window to batch events, avoiding redundant processing triggered by frequent file arrivals.
5. **The Lambda takes effect after it checks the debounce; if the debounce time has elapsed, it writes all the files as a batch under one `batch_id` and starts the Step Function.**
Once the debounce window expires without new events, the Lambda groups the related files into a batch chunk, assigns a unique `batch_id`, writes this batch metadata into the batch chunks DynamoDB table, and immediately triggers the Step Functions workflow with this batch information.
6. **The validation task starts with the files from the batch and writes to the Delta Lake.**
The Step Functions state machine kicks off an ECS Fargate Spark job that validates the batch files. This job reads the raw data from S3, performs data quality checks (e.g., nulls, referential integrity), and writes the validated data into Delta Lake format stored back in S3.
7. **A Lambda function passes the success message to the transformation task which runs and writes to the KPI DynamoDB table.**
After validation completes, a Lambda function reads the validation result (success or failure) from S3 and signals the next Step Functions state. The transformation ECS Spark job then runs, processing the validated data and writing KPI or aggregated results into a dedicated DynamoDB table for downstream analytics.
8. *(Implicit)* The transformation task completes writing data and signals success or failure.
9. *(Implicit)* Step Functions handles retries or error notifications if any task fails.
10. **The KPI DynamoDB table stores the transformed and aggregated results for reporting and analytics.**
11. **A Lambda updates the batch status to `done` and then ends the Step Function.**
Upon successful completion of all tasks, a Lambda updates the batch chunk item in DynamoDB to mark it as `done`, signaling that processing is complete. The Step Functions execution then ends, freeing the pipeline to process new batches.
